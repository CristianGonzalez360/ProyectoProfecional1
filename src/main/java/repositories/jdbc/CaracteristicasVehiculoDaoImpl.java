...
public class CaracteristicasVehiculoDaoImpl extends GenericJdbcDao<CaracteristicaVehiculoDTO>
		implements CaracteristicasVehiculoDao {

	private static final String insert = "INSERT INTO CaracteristicasVehiculo(cilindrada, motor,direccion,potencia,frenosDelanteros,transmision,frenosTraseros,torqueMaximo,volumenDeBaul,nroDePuertas, precio) VALUES (?,?,?,?,?,?,?,?,?,?,?)";
	...
	public boolean insert(CaracteristicaVehiculoDTO entity) {
		return getTemplate().query(insert).param(entity.getCilindrada()).param(entity.getMotor())
				.param(entity.getDireccion()).param(entity.getPotencia()).param(entity.getFrenosDelanteros())
				.param(entity.getTransmision()).param(entity.getFrenosTraseros()).param(entity.getTorqueMaximo())
				.param(entity.getVolumenBaul()).param(entity.getCantidadPuertas()).param(entity.getPrecio()).excecute();
	}
