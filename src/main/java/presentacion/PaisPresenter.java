...
public class PaisPresenter {
	...
	private void onDisplayFormForSave(ActionEvent a) {
		String input = new InputDialog().title("Ingrese los datos del nuevo país").open();
	...
	private void onDisplayFormForUpdate(ActionEvent a) {
		...
		if (current != null) {
			String input = new InputDialog().title("Ingrese los nuevos datos del nuevo pais")
					.setText(current.getNombre()).open();
			if (input != null && !input.trim().isEmpty()) {
				try {
					PaisDTO target = new PaisDTO(null, input);
